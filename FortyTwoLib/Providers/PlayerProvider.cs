...
{
	public class PlayerProvider : MembershipProvider
	{
		public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
		{
			using ( var db = new FortyTwoDataContext(ConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString) )
			{
				...
				catch (Exception ex)
				{
					...
					Server.GetInstance().Log.WriteError("Unable to create new player.", ex);
					return null;
				}
			}
		}

		private MembershipUser GetMemberShipUser(Database.Player player)
		{
